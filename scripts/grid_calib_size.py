#!/usr/bin/env python3
"""校准规模分析 — 隔离网格: 固定谱 (全部 train 查询), 只变网格校准查询数.

问题: relevance-calibrated grid 需要多少 judged queries 才饱和?
协议: scifact e5, keep=50% B=2. margin_importance 固定用全部 809 train 查询;
run_reco 的网格拟合 (fit_calib_levels) 只用前 N 个 train 查询.
评估 test (零泄漏). 对照: 全管线同规模 (谱+网格一起变, calib_size_reco.py).
用法: python grid_calib_size.py --model e5 --ds_dir data/beir/scifact --out X.json
"""
...
def main():
    ...
    ap.add_argument("--out", default="/tmp/grid_calib_size.json")
    ap.add_argument("--keep", type=float, default=0.5)
    ap.add_argument("--fracs", nargs="+", type=float,
                    default=[0.0, 0.03, 0.06, 0.1, 0.25, 0.5, 0.7, 1.0])
    args = ap.parse_args()
    ...
    res = {"ds_dir": args.ds_dir, "model": args.model, "keep": args.keep, "B": B,
           ...
           "prefix": round(prefix, 4), "grid_only": True, "fracs": {}}
